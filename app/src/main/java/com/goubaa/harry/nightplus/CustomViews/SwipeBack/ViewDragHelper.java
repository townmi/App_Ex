package com.goubaa.harry.nightplus.CustomViews.SwipeBack;
...
public class ViewDragHelper {
   ...
   * callback's
   * {@link com.goubaa.harry.nightplus.CustomViews.SwipeBack.ViewDragHelper.Callback#onEdgeTouched(int, int)}
   * and
   * {@link com.goubaa.harry.nightplus.CustomViews.SwipeBack.ViewDragHelper.Callback#onEdgeDragStarted(int, int)}
   ...
   * {@link com.goubaa.harry.nightplus.CustomViews.SwipeBack.ViewDragHelper.Callback#tryCaptureView(View, int)}
  ...
  public boolean settleCapturedViewAt(int finalLeft, int finalTop) {
    ...
    return forceSettleCapturedViewAt(finalLeft, finalTop, (int) VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId), (int) VelocityTrackerCompat.getYVelocity(mVelocityTracker,
      mActivePointerId));
  }
  ...
  public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
    ...
    mScroller.fling(mCapturedView.getLeft(), mCapturedView.getTop(), (int) VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId), (int) VelocityTrackerCompat.getYVelocity(mVelocityTracker,
      mActivePointerId), minLeft, maxLeft, minTop, maxTop);
  ...
  protected boolean canScroll(View v, boolean checkV, int dx, int dy, int x, int y) {
    if (v instanceof ViewGroup) {
      ...
      for (int i = count - 1; i >= 0; i--) {
        ...
        if (x + scrollX >= child.getLeft() && x + scrollX < child.getRight() && y + scrollY >= child.getTop() && y + scrollY < child.getBottom() && canScroll(child, true, dx, dy, x + scrollX - child.getLeft
  ...
  private boolean checkNewEdgeDrag(float delta, float odelta, int pointerId, int edge) {
    ...
    if ((mInitialEdgeTouched[pointerId] & edge) != edge || (mTrackingEdges & edge) == 0 || (mEdgeDragsLocked[pointerId] & edge) == edge || (mEdgeDragsInProgress[pointerId] & edge) == edge || (absDelta <=
      mTouchSlop && absODelta <= mTouchSlop)) {
      return false;
    }
   ...
   * {@link com.goubaa.harry.nightplus.CustomViews.SwipeBack.ViewDragHelper.Callback#getOrderedChildIndex(int)}
   * .
   *
   * @param x X position to test in the parent's coordinate system
   * @param y Y position to test in the parent's coordinate system
