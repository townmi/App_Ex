...
public class RetrofitFactory {
  ...
  private static OkHttpClient httpClient = new OkHttpClient.Builder().addInterceptor(new
                                                                                       Interceptor() {
    @Override
    public Response intercept(Chain chain) throws IOException {
      Request.Builder builder = chain.request().newBuilder();
      builder.addHeader("Authorization", "Bearer " +
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE1MTgxNDY0OTcsImlzcyI6ImUzZWQwOWQwLWVhMDgtMTFlNy1hYzE0LTZmMDVmZDhlNTU1OSIsImlhdCI6MTUxNzU0MTY5N30.1UBq89-AyxltTs0aovdL-2JHF1Vv0QzRgQMDDzV-w8w");
  ...
  private static RetrofitService retrofitService = new Retrofit.Builder().baseUrl(BASE_URL)
    .addConverterFactory(GsonConverterFactory.create(buildGson())).addCallAdapterFactory
      (RxJava2CallAdapterFactory.create()).client(httpClient).build().create(RetrofitService.class);

  // venuesCore service
  private static RetrofitService venuesCoreRetrofitService = new Retrofit.Builder().baseUrl
    (VENUES_CORE_BASE_URL).addConverterFactory(GsonConverterFactory.create(buildPostGson()))
    .addCallAdapterFactory(RxJava2CallAdapterFactory.create()).client(httpClient).build().create
      (RetrofitService.class);

  // community core service
  private static RetrofitService communityCoreRetrofitService = new Retrofit.Builder().baseUrl
    (COMMUNITY_CORE_BASE_URL).addConverterFactory(GsonConverterFactory.create(buildPostGson()))
    .addCallAdapterFactory(RxJava2CallAdapterFactory.create()).client(httpClient).build().create
      (RetrofitService.class);

  // user model
  private static RetrofitService userRetrofitService = new Retrofit.Builder().baseUrl
    (USER_BASE_URL).addConverterFactory(GsonConverterFactory.create(buildUserGson()))
    .addCallAdapterFactory(RxJava2CallAdapterFactory.create()).client(httpClient).build().create
      (RetrofitService.class);
  ...
  // home posts model
  private static RetrofitService explorePostsRetorfitService = new Retrofit.Builder().baseUrl
    (SEARCH_BASE_URL).addConverterFactory(GsonConverterFactory.create(buildExplorePostsGson()))
    .addCallAdapterFactory(RxJava2CallAdapterFactory.create()).client(httpClient).build().create
      (RetrofitService.class);
  ...
  private static Gson buildGson() {
    return new GsonBuilder().serializeNulls().setFieldNamingPolicy(FieldNamingPolicy.IDENTITY)
      .registerTypeAdapter(City.class, new CityTypeAdapter()).create();
  }

  @NonNull
  private static Gson buildPostGson() {
    return new GsonBuilder().serializeNulls().setFieldNamingPolicy(FieldNamingPolicy.IDENTITY)
      .registerTypeAdapter(Post.class, new PostTypeAdapter()).create();
  }
  ...
  private static Gson buildExplorePostsGson() {
    return new GsonBuilder().serializeNulls().setFieldNamingPolicy(FieldNamingPolicy.IDENTITY)
      .registerTypeAdapter(ExprolePosts.class, new ExplorePostsAdapter()).create();
  }
}
