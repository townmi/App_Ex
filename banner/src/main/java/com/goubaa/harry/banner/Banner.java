...
import com.goubaa.harry.banner.listener.OnBannerClickListener;
import com.goubaa.harry.banner.listener.OnBannerListener;
import com.goubaa.harry.banner.loader.ImageLoaderInterface;
import com.goubaa.harry.banner.view.BannerViewPager;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
  ...
  private void handleTypedArray(Context context, AttributeSet attrs) {
    ...
    mIndicatorHeight = typedArray.getDimensionPixelSize(R.styleable.Banner_indicator_height,
      indicatorSize);
    mIndicatorSelectedWidth = typedArray.getDimensionPixelSize(R.styleable
      .Banner_indicator_selected_width, indicatorSize);
    mIndicatorSelectedHeight = typedArray.getDimensionPixelSize(R.styleable
      .Banner_indicator_selected_height, indicatorSize);
    mIndicatorMargin = typedArray.getDimensionPixelSize(R.styleable.Banner_indicator_margin,
      BannerConfig.PADDING_SIZE);
    mIndicatorSelectedResId = typedArray.getResourceId(R.styleable
      .Banner_indicator_drawable_selected, R.drawable.gray_radius);
    mIndicatorUnselectedResId = typedArray.getResourceId(R.styleable
      .Banner_indicator_drawable_unselected, R.drawable.white_radius);
    ...
    titleBackground = typedArray.getColor(R.styleable.Banner_title_background, BannerConfig
      .TITLE_BACKGROUND);
    titleHeight = typedArray.getDimensionPixelSize(R.styleable.Banner_title_height, BannerConfig
      .TITLE_HEIGHT);
    titleTextColor = typedArray.getColor(R.styleable.Banner_title_textcolor, BannerConfig
      .TITLE_TEXT_COLOR);
    ...
    bannerBackgroundImage = typedArray.getResourceId(R.styleable.Banner_banner_default_image, R
      .drawable.no_banner);
    typedArray.recycle();
  }
  ...
  private void setTitleStyleUI() {
    ...
    if (titleHeight != -1) {
      titleView.setLayoutParams(new RelativeLayout.LayoutParams(ViewGroup.LayoutParams
  ...
  private void initImages() {
    imageViews.clear();
    if (bannerStyle == BannerConfig.CIRCLE_INDICATOR || bannerStyle == BannerConfig
      .CIRCLE_INDICATOR_TITLE || bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE_INSIDE) {
      createIndicator();
...
//        Log.i(tag, ev.getAction() + "--" + isAutoPlay);
    if (isAutoPlay) {
      ...
      if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL || action ==
...
//        Log.i(tag,"currentItem: "+currentItem);
  ...
  public void onPageSelected(int position) {
    ...
    if (bannerStyle == BannerConfig.CIRCLE_INDICATOR || bannerStyle == BannerConfig
      ...
      LinearLayout.LayoutParams Unselectedparams = new LinearLayout.LayoutParams(mIndicatorWidth,
        mIndicatorHeight);
      Unselectedparams.leftMargin = mIndicatorMargin;
      Unselectedparams.rightMargin = mIndicatorMargin;

      indicatorImages.get((lastPosition - 1 + count) % count).setImageResource
